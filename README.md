...
┌─────────────────────────┐
                ...
                └────────────┬────────────┘
                             │
                 Stage 1: P_DA Commitment
                             │
                             ▼
┌─────────────────────────────────────────────────────────────────┐
│               Real-Time Co-Located Facility Dispatch            │
│                                                                 │
│   ┌────────────────┐      ┌────────────────┐      ┌─────────┐   │
│   │ PV Generation  │ ───► │  BESS Storage  │ ───► │ Grid POI│   │
│   │   (Stochastic) │      │ (DoD Piecewise)│      │ Export  │   │
│   └────────────────┘      └────────────────┘      └────┬────┘   │
└────────────────────────────────────────────────────────┼────────┘
│
Imbalance ΔP = P_net - P_DA
│
▼
┌─────────────────────────┐
│ reBAP Imbalance Pricing │
│ + CVaR Risk Mitigation  │
└─────────────────────────┘
...
$$\max_{\Xi} \; \mathbb{E}[\text{Revenue}] - \mathbb{E}[\text{Degradation Cost}] + \beta \cdot \text{CVaR}_\alpha$$
...
Nonlinear cell aging is linearized via $K$ piecewise intervals over battery throughput:
...
2. Execute Risk-Constrained Optimization
Python
...
📚 Citation
If you utilize this framework or reference the methodology, please cite the working paper:
